...
//
// LinReg_fAPF.cpp: Example 5.2 of Guarniero, Johansen and Lee (2016):
...
#include "LinGauss_fAPF.h"

using namespace std;
using namespace linGauss_fAPF;

// linGauss_fAPF_impl() function callable from R via Rcpp::
// [[Rcpp::export]]
Rcpp::List linGauss_fAPF_impl(arma::mat data, arma::rowvec initial, unsigned long lNumber, unsigned long lMCMCits) {    
    ...
    try {
        ...
        prep = arma::inv(I + 0.25*I);
        SIG = prep*0.25;
        cholSIG = arma::chol(SIG);
...
namespace linGauss_fAPF {
    ...
    /// \param X            A reference to the empty particle class
    ...
    {
        X = Rcpp::as<arma::vec>(Rcpp::rnorm(lDim));
        logweight = logMvnPdf(y.row(0).t(), X, I + 0.25*I);
    }

    ///The proposal function.

    ///\param lTime     The sampler iteration.
    ///\param X         A reference to the current particle class
    ///\param params    A reference to the (null) parameter values
    void fMove(long lTime, arma::vec & X, double & logweight, smc::nullParams & params)
    {
        arma::vec mu = (0.25*prep*(A*X)) + (prep*y.row(lTime-1).t()); //the mean for the product of two MVN's, N(y.row(lTime-1),SIGe) and N(A*X.value,SIGv)
        X = mu + cholSIG*Rcpp::as<arma::vec>(Rcpp::rnorm(lDim));

        logweight += logMvnPdf(y.row(lTime).t(),A*X,I + 0.25*I);
    }
}
