...
Rcpp::DataFrame nonLinPMMH_impl(arma::vec data, unsigned long lNumber, unsigned long lMCMCits) {
    ...
    try {
        ...
        smc::sampler<double,params> Sampler(lNumber, HistoryType::NONE);
        ...
        smc::moveset<double,params> Moveset(fInitialise, fMove, NULL);
...
namespace nonLinPMMH {
    ...
    void fInitialise(double & X, double & logweight, params & param)
    {
    ...
    /// \param param        Additional algorithm parameters
    void fMove(long lTime, double & X, double & logweight, params & param)
    {
