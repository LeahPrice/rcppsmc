...
Rcpp::List LinRegLA_adapt_impl(arma::mat Data, unsigned long lNumber, double resampTol, double tempTol) {     
    ...
    try {
        ...
        smc::sampler<rad_state,smc::staticModelAdapt> Sampler(lNumber, HistoryType::RAM);
        ...
        temps = Sampler.GetAlgParams().GetTemps();
...
namespace LinReg_LA_adapt {
    ...
    double width_ps(long lTime, void *){
        return (temps[lTime] - temps[lTime-1]); //can't access Sampler here so temps set separately
    }   
